...
import { isVideoVisibleAtTime } from './videoVisibility'
...
describe('split video visibility', () => {
  it('shows only the active clip, including while scrubbing backwards', () => {
    ...
    for (const [time, active] of [[0, 0], [4, 1], [8, 2], [11.9, 2], [5, 1], [1, 0]]) {
      expect(clips.map(n => isVideoVisibleAtTime(n, time))).toEqual(clips.map((_, i) => i === active))
    }
    expect(clips.some(n => isVideoVisibleAtTime(n, 12))).toBe(false)
  })
  it('honors trim, speed, and a negative start after a scene split', () => {
    expect(isVideoVisibleAtTime(clip, 9.99)).toBe(false)
    expect(isVideoVisibleAtTime(clip, 10)).toBe(true)
    expect(isVideoVisibleAtTime(clip, 13.99)).toBe(true)
    expect(isVideoVisibleAtTime(clip, 14)).toBe(false)
    expect(isVideoVisibleAtTime({ ...clip, startTime: -2 }, 0)).toBe(true)
    expect(isVideoVisibleAtTime({ ...clip, startTime: -2 }, 2)).toBe(false)
  })
  it('preserves frame holds for videos without the opt-in setting', () => {
    for (const time of [-10, 0, 100]) {
      expect(isVideoVisibleAtTime({ ...clip, clipToRange: undefined }, time)).toBe(true)
      expect(isVideoVisibleAtTime({ ...clip, clipToRange: false }, time)).toBe(true)
    }
  })
  it('reads legacy stored flags and preserves them through save and copy', () => {
    ...
    expect(isVideoVisibleAtTime(video, 0)).toBe(false)
