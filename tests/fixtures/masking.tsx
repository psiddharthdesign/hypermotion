...
function fixture(ellipse = false, nested = false) {
  ...
  const mask = api.createNode(ellipse ? 'ellipse' : 'rect', group, { isMask: true, size: { width: 160, height: 140 }, appearance: appearance('#ff00ff') })
...
export function App() {
  ...
  return <main style={{ font: '14px system-ui', padding: 20 }}>
    ...
    <p>Content X: {contentX} · Mask X: {maskX} · Mask rotation: {rotation} · {final ? 'Export' : 'Preview'} · {fallback ? 'DOM' : 'GPU'} {fallback || available ? 'ready' : 'loading'}</p>
      ...
      {fixtures.map((f, i) => <section key={i}><h2>{['Rectangle', 'Ellipse', 'Nested rectangle'][i]}</h2>
        <div style={{ position: 'relative', width: 400, height: 260, background: '#fff', border: '1px solid #ddd' }}>
          {fallback ? <SceneContext.Provider value={f.api}><SceneLayer sceneApi={f.api} rootId={f.root}
            solved={f.layout} order={Object.keys(f.layout)} animated={{[f.content]:{x:contentX},[f.mask]:{x:maskX,rotation}}}
            inherited={composeInheritedAnim(f.api,f.root,{[f.content]:{x:contentX},[f.mask]:{x:maskX,rotation}},f.layout)} /></SceneContext.Provider> : <ThreeSceneViewport api={f.api} layout={f.layout} camera={f.camera} cameraAnim={{x:200,y:130,z:0}}
            animated={{[f.content]:{x:contentX},[f.mask]:{x:maskX,rotation}}} width={400} height={260}
